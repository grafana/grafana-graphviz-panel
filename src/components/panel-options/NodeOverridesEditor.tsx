...
import { collectAllNodeIds } from '../../core/utils/graphvizAst';
    ...
    if (mapping) {
      ...
      if (rule.kind === RuleKind.TOOLTIP) {
        const newLink = {
          ...
          url: 'https://example.com?node=${__nodeId}',
          openInNewTab: true,
        };
          ...
          >
                ...
                {rule.kind === RuleKind.TOOLTIP && (
                  <>
                    <FieldSet label="Header">
                      ...
                      <InlineField label="Show node ID" labelWidth={20}>
                        <InlineSwitch
                    ...
                    <FieldSet label="Content">
                      <Field
                        ...
                        description="Use ${field_name} for data fields, ${__nodeId} for node ID"
                      >
                        <TextArea
                        ...
                        <div style={{ marginBottom: 12 }}>
                          {rule.footer.links.map((link, linkIndex) => (
                            ...
                            >
                              ...
                              <Field
                                label="URL"
                                description="Use ${field_name} for data, ${__nodeId} for node ID, ${var-name} for dashboard variables"
                              >
                                <Input
                                  value={link.url}
                                  placeholder="https://example.com?node=${__nodeId}"
                                  onChange={(e) =>
                                    updateDataLinkInTooltip(mapping.id, ruleIndex, linkIndex, {
            ...
            <Box marginTop={1.5}>
              {(() => {
                ...
                const isButtonDisabled = !hasMatchedNodes;
...
function extractNodeIds(dotDiagram: string | undefined): string[] {
  ...
  try {
    ...
    console.error('Error extracting node IDs from DOT:', error);
    return [];
  }
}
